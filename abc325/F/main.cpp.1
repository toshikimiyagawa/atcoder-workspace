...
const ll INF = 1e18;
// clang-format on
void solve(long long N, std::vector<long long> D, std::vector<long long> L, std::vector<long long> C, std::vector<long long> K) {
...
int main() {
    ...
    std::vector<long long> K(2);
    for(int i = 0 ; i < 2 ; i++){
        cin >> L[i];
        cin >> C[i];
        cin >> K[i];
    }
    solve(N, std::move(D), std::move(L), std::move(C), std::move(K));
    return 0;
}
