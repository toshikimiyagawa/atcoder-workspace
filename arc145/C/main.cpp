...
void solve(long long N) {
    ...
    rep(i, N) {
        vector<mint> dp1(i + 1);
        vector<mint> dp2(i + 1);
        reps(j, max(0LL, 2 * i - N), i) {
            if (j == 0) {
                dp[2 * i][j] = dp[2 * i - 1][2 * i - j - 1];
            } else {
                dp[2 * i][j] = dp[2 * i - 1][j - 1] + dp[2 * i - 1][2 * i - j - 1];
            }
            dp[2 * i][2 * i - j] = dp[2 * i][j];
        }
        dp[2 * i][i] = dp[2 * i - 1][i - 1];
        reps(j, max(0LL, 2 * i - N), i + 1) {
            if (j == 0) {
                dp[2 * i + 1][j] = dp[2 * i - 1][2 * i - j];
            } else {
                dp[2 * i + 1][j] = dp[2 * i - 1][j - 1] + dp[2 * i - 1][2 * i - j];
            }
        }
    }
    cout << dp[2 * N + 1][N].val() << endl;
}
