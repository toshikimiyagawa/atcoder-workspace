...
void solve(long long N, long long K, long long T, std::vector<std::vector<long long>> B) {
...
int main() {
    ...
    long long T;
    cin >> T;
    std::vector<std::vector<long long>> B(N, std::vector<long long>(K));
    for(int i = 0 ; i < N ; i++){
        for(int j = 0 ; j < K ; j++){
            cin >> B[i][j];
        }
    }
    solve(N, K, T, std::move(B));
    return 0;
}
