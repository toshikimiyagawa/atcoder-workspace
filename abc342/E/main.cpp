...
int main() {
    ...
    map<ll, vector<tuple<ll, ll, ll, ll, ll>>> p;
    rep(i, M) {
        cin >> l[i] >> d[i] >> k[i] >> c[i] >> A[i] >> B[i];
        p[B[i]].pb({A[i], l[i], d[i], k[i], c[i]});
    }
    ...
    while (true) {
        rep(i, p[pp].size()) {
            ...
            ll kk = get<3>(p[pp][i]);
            ll cc = get<4>(p[pp][i]);
            ll ttt = min(t - cc - (t - cc - lll) % dd, lll + (kk - 1) * dd);
            // cout << AA << " " << lll << " " << dd << " " << kk << " " << cc
            //      << " " << ttt << " " << tmp[AA - 1] << endl;
            if (AA != N && tmp[AA - 1] < ttt && t >= lll + cc) {
                // cout << ttt << endl;
                time.erase({tmp[AA - 1], AA});
                tmp[AA - 1] = ttt;
                time.insert({ttt, AA});
            }
        }
