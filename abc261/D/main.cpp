...
void solve(long long N, long long M, std::vector<long long> X, std::vector<long long> C, std::vector<long long> Y){
...
int main(){
    ...
    for(int i = 0 ; i < M ; i++){
        (void) std::scanf("%lld", &C[i]);
        (void) std::scanf("%lld", &Y[i]);
    }
    solve(N, M, std::move(X), std::move(C), std::move(Y));
    return 0;
}
