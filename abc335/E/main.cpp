...
void solve(long long N, long long M, std::vector<long long> A,
    ...
    rep(i, M) {
        if (A[U[i] - 1] > A[V[i] - 1]) {
            vv[d.leader(V[i] - 1)].pb(d.leader(U[i] - 1));
        }
        if (A[U[i] - 1] < A[V[i] - 1]) {
            vv[d.leader(U[i] - 1)].pb(d.leader(V[i] - 1));
        }
    }
    ...
    x[d.leader(0)] = 1;
    rep(i, aa.size() - 1) {
        rep(j, vv[aa[i].second].size()) {
            if (x[vv[aa[i].second][j]] < x[aa[i].second] + 1 &&
                x[aa[i].second] > 0) {
                x[vv[aa[i].second][j]] = x[aa[i].second] + 1;
            }
        }
    }
    // print(x);
    cout << x[d.leader(N - 1)] << endl;
}
