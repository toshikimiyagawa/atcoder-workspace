...
void solve(long long N, std::vector<long long> P) {
    ...
    rep(i, N / 2) {
        if (P[2 * i + 1] % 2 == 1) {
            for (int j = 2 * i - 1; j >= x; j -= 2) {
                ll t = P[j];
                P[j] = P[j + 2];
                P[j + 2] = t;
                ret.pb(make_pair('B', j + 1));
            }
            x += 2;
        }
    }
...
//    print(P);
    ...
    rep(i, (N + 1) / 2) {
        rep(j, (N + 1) / 2) {
            if (P[2 * j] == 2 * i + 1) {
                for (int k = 2 * j - 2; k >= 2 * i; k -= 2) {
                    ll t = P[k];
                    P[k] = P[k + 2];
                    P[k + 2] = t;
                    ret.pb(make_pair('B', k + 1));
                }
            }
        }
    }
