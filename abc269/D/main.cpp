...
void solve(long long N, std::vector<long long> X, std::vector<long long> Y) {
    ...
    dsu d(N);
    rep(i, N) {
        if (z.count(X[i] - 1) && z[X[i] - 1].count(Y[i] - 1)) {
            d.merge(z[X[i] - 1][Y[i] - 1], i);
        }
        if (z.count(X[i] - 1) && z[X[i] - 1].count(Y[i])) {
            d.merge(z[X[i] - 1][Y[i]], i);
        }
        if (z.count(X[i]) && z[X[i]].count(Y[i] - 1)) {
            d.merge(z[X[i]][Y[i] - 1], i);
        }
        if (z.count(X[i] + 1) && z[X[i] + 1].count(Y[i] + 1)) {
            d.merge(z[X[i] + 1][Y[i] + 1], i);
        }
