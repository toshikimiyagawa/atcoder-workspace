...
int main() {
    int N;
    cin >> N;
    ll x_min = 1, x_max = N, y_min = 1, y_max = N;
    while (x_min != x_max) {
        cout << "? " << x_min << " " << x_min + (x_max - x_min) / 2 << " " << 1 << " " << N << endl;
        int xx;
        cin >> xx;
        if (xx == (x_max - x_min) / 2 + 1) {
            x_min = x_min + (x_max - x_min) / 2 + 1;
        } else {
            x_max = x_min + (x_max - x_min) / 2;
        }
    }
    while (y_min != y_max) {
        cout << "? " << 1 << " " << N << " " << y_min << " " << y_min + (y_max - y_min) / 2 << endl;
        int xx;
        cin >> xx;
        if (xx == (y_max - y_min) / 2 + 1) {
            y_min = y_min + (y_max - y_min) / 2 + 1;
        } else {
