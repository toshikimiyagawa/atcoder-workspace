...
mint s(ll aa, ll bb, ll cc, ll dd, ll mm) {
    ll count = (bb - aa + 1) * (dd - cc + 1);
    if (count % 2 == 1 && (aa + cc) % 2 == 0) {
        count++;
    }
    count /= 2;
    ll e = (aa - 1) * mm + cc + (bb - 1) * mm + dd;
//    if (count % 2 == 0) {
//        count /= 2;
//    } else {
//        e /= 2;
//    }
    mint c = count;
    mint ee = e;
//    cout << "s=" << (c * ee).val() << endl;
    return (c * ee) / 2;
}

void solve(long long N, long long M, long long Q, std::vector<long long> A, std::vector<long long> B,
           std::vector<long long> C, std::vector<long long> D) {
    std::cout << std::fixed << std::setprecision(10);
    rep(i, Q) {
        mint ret;
        if ((B[i] - A[i]) % 2 == (D[i] - C[i]) % 2) {
            ret = s(A[i], B[i], C[i], D[i], M);
        } else if ((B[i] - A[i]) % 2 == 0) {
            ret = s(A[i], B[i], C[i], D[i] - 1, M)
                  + s(A[i], B[i], D[i], D[i], M);
        } else {
            ret = s(A[i], B[i] - 1, C[i], D[i], M)
                  + s(B[i], B[i], C[i], D[i], M);
        }
        cout << ret.val() << endl;
    }
