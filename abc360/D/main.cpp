...
const ll INF = 1e18;
// clang-format on
void solve(long long N, long long T, std::string S, std::vector<long long> X) {
    ...
    ll ret = 0;
    rep(i, start.size()) {
        while (a < end.size() && end[a] < start[i]) {
            a++;
        }
        while (b < end.size() && end[b] <= start[i] + T * 2) {
            b++;
        }
