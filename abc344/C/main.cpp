...
const string NO = "No";
// clang-format on
void solve(long long N, std::vector<long long> A, long long M,
           std::vector<long long> B, long long L, std::vector<long long> C,
           long long Q, std::vector<long long> X) {
    std::cout << std::fixed << std::setprecision(10);
    set<ll> x;
    rep(i, N) {
        rep(j, M) {
            rep(k, L) { x.insert(A[i] + B[j] + C[k]); }
        }
    }
    rep(i, Q) {
        if (x.find(X[i]) == x.end()) {
