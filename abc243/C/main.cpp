...
void solve(long long N, std::vector<long long> X, std::vector<long long> Y, std::string S) {
    ...
    string ret = "No";
    rep(i, N) {
        if (r_min.count(Y[i]) && S[i] == 'L' && X[i] > r_min[Y[i]]) {
//            cout << "a" << endl;
            ret = "Yes";
        }
        if (l_max.count(Y[i]) && S[i] == 'R' && X[i] < l_max[Y[i]]) {
//            cout << "b" << endl;
            ret = "Yes";
        }
        if (S[i] == 'L' && (l_max.count(Y[i]) == 0 || X[i] > l_max[Y[i]])) {
//            cout << "c" << endl;
            l_max[Y[i]] = X[i];
        }
        if (S[i] == 'R' && (r_min.count(Y[i]) == 0 || X[i] < r_min[Y[i]])) {
//            cout << "d" << endl;
            r_min[Y[i]] = X[i];
        }
    }
