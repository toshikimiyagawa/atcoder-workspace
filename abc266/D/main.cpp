...
void solve(long long N, std::vector<long long> T, std::vector<long long> X, std::vector<long long> A) {
    vvll dp(T[N - 1] + 1);
    rep(i, T[N - 1] + 1) {
        dp[i].resize(5);
    }
    ...
    cout << max(dp[T[N - 1]][0], max(dp[T[N - 1]][1], max(dp[T[N - 1]][2], max(dp[T[N - 1]][3], dp[T[N - 1]][4]))))
         << endl;
}
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        (void) std::scanf("%lld", &T[i]);
        (void) std::scanf("%lld", &X[i]);
        (void) std::scanf("%lld", &A[i]);
    }
    solve(N, std::move(T), std::move(X), std::move(A));
    return 0;
}
