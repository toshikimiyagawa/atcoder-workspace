...
void solve(long long N, long long M, std::vector<long long> u,
           std::vector<long long> v, std::vector<long long> W,
    ...
    rep(i, N) {
        ...
        ll idx = x[i].second;
        // cout << ww << endl;
        vvll dp(b[idx].size() + 1, vll(ww + 1, 0));
        rep(j, b[idx].size()) {
            rrep(k, ww) {
                // cout << W[b[idx][j]] << endl;
                // cout << j << " : " << k << endl;
                ll xx = 0;
                if (W[b[idx][j]] <= k) {
                    xx = dp[j][k - W[b[idx][j]]] + NN[b[idx][j]];
                }
                dp[j + 1][k] = max(dp[j][k], xx);
                // rep(i, b[idx].size()) { print(dp[i]); }
            }
        }
        NN[idx] = dp[b[idx].size()][ww] + 1;
        // cout << "NN :";
        // print(NN);
    }
    // print(NN);
    ll ret = 0;
    rep(i, N) { ret += A[i] * NN[i]; }
    cout << ret << endl;
}
...
int main() {
    ...
    solve(N, M, std::move(u), std::move(v), std::move(W), std::move(A));
    return 0;
}
